...
check_valid_download  () {
    ...
    "$SZ" t "$(cygpath -wa "$GDC_DL_PATH/$GDC_FILE_NAME")"
}
wget_latest_driver () {
...
wget_szip () {
    ...
    wget -U "$GDC_WGET_USR_AGENT" --no-cookies -N --no-check-certificate -P "$GDC_DL_PATH" "$url" && { exec_msi_szip || return 1; } || return 1
    find_path_szip
}
get_array_deps () {
